# M620 - Documentação Completa de Relacionamentos

## 📊 Informações Gerais

- **Nome da Tabela**: M620 (Ajustes de Apuração de ICMS)
...
**M620** é uma tabela que armazena informações sobre ajustes de apuração de ICMS para o SPED Fiscal. Com **178 registros**, representa ajustes realizados em apurações de ICMS, incluindo informações sobre tipo de ajuste, valor, código e descrição.

Esta tabela funciona como **detalhamento de ajustes de apuração de ICMS** e permite:
- Registrar ajustes de apuração de ICMS
- Armazenar informações sobre tipo, valor e código de ajuste
- Vincular ajustes a lançamentos de PIS/COFINS (compartilhando estrutura com M220)
...
Cada registro representa um ajuste específico de apuração de ICMS, contendo:
- Código do ajuste (MCODIGO)
- Código do lançamento de PIS/COFINS (LCCODIGO) - compartilhado com M220
...
O sistema utiliza esta tabela para manter histórico completo de ajustes de apuração de ICMS, sendo referenciada por LCPISCOFINS através de LCCODIGO e EMPCODIGO para vincular ajustes a lançamentos específicos.

**Observação Importante:** M620 é uma tabela de ajustes de apuração de ICMS para SPED Fiscal. Com 178 registros, indica uso moderado desta funcionalidade. Possui chave primária composta (MCODIGO, EMPCODIGO) e referencia LCPISCOFINS através de LCCODIGO e EMPCODIGO. Note que M620 compartilha a mesma estrutura de relacionamento com LCPISCOFINS que M220, possivelmente indicando uma estrutura compartilhada para diferentes tipos de ajustes fiscais.
...
**Foreign Keys:**
- `FK_M620_1`: LCCODIGO, EMPCODIGO → LCPISCOFINS.LCCODIGO, LCPISCOFINS.EMPCODIGO

---

## 🔗 Relacionamentos - Nível 1 (Diretos)

### M620 Referencia (1 tabela):

#### 1. LCPISCOFINS - Lançamentos de PIS/COFINS
**Relacionamento:**
```
M620.LCCODIGO, M620.EMPCODIGO → LCPISCOFINS.LCCODIGO, LCPISCOFINS.EMPCODIGO (N:1)
Constraint: FK_M620_1
```
...
### M620 é Referenciada Por (0 tabelas):

Nenhuma tabela referencia M620 diretamente.
...
**Fluxo:** M620 → LCPISCOFINS → TPBLOCO → Operações
...
| Tabela | Registros | Proporção com M620 | Tipo |
|--------|-----------|-------------------|------|
| **M620** | 178 | 1:1 | **TABELA PRINCIPAL** |
| LCPISCOFINS | 356 | 1:2 | Lançamentos (média de 2 ajustes por lançamento) |

**Interpretação:**
- **178 ajustes** de apuração de ICMS registrados no sistema
...
CREATE INDEX IDX_M620_LANCAMENTO ON M620(LCCODIGO, EMPCODIGO);

-- Índice 2: Busca por NFe (consultas frequentes)
CREATE INDEX IDX_M620_NFE ON M620(NFECODIGO)
    WHERE NFECODIGO IS NOT NULL;

-- Índice 3: Busca por tipo de ajuste (consultas frequentes)
CREATE INDEX IDX_M620_TIPO_AJUSTE ON M620(MCOD_AJ)
    WHERE MCOD_AJ IS NOT NULL;

-- Índice 4: Busca por data de operação (consultas frequentes)
CREATE INDEX IDX_M620_DATA_OPER ON M620(MDT_OPER)
    WHERE MDT_OPER IS NOT NULL;
```
...
final class FirebirdM620 extends BaseFirebirdModel
{
    protected $connection = 'firebird';
    protected $table = 'M620';
